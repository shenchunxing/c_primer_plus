...
const char * colors2[8] = {"black", "red", "green", "yellow",
...
union Views     /* 联合：把数据看做结构体 or unsigned short类型的变量 ，内存是共享的 */
{
    struct box_props2 st_view;
    unsigned short us_view;
};

void show_settings0(const struct box_props2 * pb);
...
void show_settings0(const struct box_props2 * pb)
{
    ...
    printf("The fill color is %s.\n", colors2[pb->fill_color]);
    printf("Border %s.\n",
           pb->show_border == true ? "shown" : "not shown");
    printf("The border color is %s.\n", colors2[pb->border_color]);
